      ...
      >
        ...
        {isForgotView ? (
          ...
          <form onSubmit={handleSubmitLogin} className="space-y-4">
            ...
            <button
              id="btn-google-signin-user-login"
              type="button"
              onClick={async () => {
                setErrorMessage(null);
              ...
              className={`w-full py-3 px-4 rounded-xl text-sm font-semibold border transition-all flex items-center justify-center gap-2.5 min-h-[44px] ${
                theme === 'dark'
            ...
            <div className="relative flex items-center justify-center my-1">
              <div className={`border-t w-full ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`} />
              <span
                className={`px-2.5 text-[11px] uppercase tracking-wider absolute font-medium rounded ${
                  theme === 'dark' ? 'bg-[#0e121e] text-slate-400' : 'bg-white text-slate-400'
                }`}
              >
                or email
              </span>
